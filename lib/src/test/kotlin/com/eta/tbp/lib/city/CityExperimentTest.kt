...
/**
 * [CityExperiment] no longer takes its own `random` — exploration randomness
 ...
 * outcome (not just "some cellsVisited count") need a reproducible visit
 * order to avoid flaking.
 */
class CityExperimentTest {
    /**
     * Deliberately visits every one of [cityMap]'s own landmark cells (unlike [CityExperiment],
     ...
     * returned state is what a caller feeds into the next [teachManually] call, or into a
     * [CityExperiment] via [CityExperiment.loadState]. [random] is seeded by the caller — same
    ...
    fun `a shared logger receives events from every layer of the pipeline`() {
        ...
        assertEquals(setOf("CityExperiment", "Explorer", "EnvironmentSensorModule", "EvidenceGraphLM"), tags)
        assertTrue(
