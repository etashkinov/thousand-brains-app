...
| Goal-State Generators / `CmpGoal` | Live for the city tier: `EvidenceGraphLM.proposeGoal()` → `CmpGoal` → `Explorer`'s `MotorSystem`, a single-LM-to-motor loop real Monty exercises with no voting needed. Touch tier still has no motor system, so no producer/consumer there | **Faithful** for the city tier; Phase 8's own use of `CmpGoal` — voting between sibling LMs' goals — is still unbuilt |
